...
release: image
	docker run --rm -v $(CURDIR):/go/src/github.com/gotoz/runq -v /usr/bin/docker-init:/usr/bin/docker-init:ro runq-build make RUNQ_ROOT=$(RUNQ_ROOT) clean install tarfile clean2
